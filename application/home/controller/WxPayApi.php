...
use app\home\controller\WxPayException;
...
class WxPayApi
{
	...
	{
		//如果不需要上报数据
		if(config('wechat_report_levenl') == 0){
			return;
		} 
		//如果仅失败上报
		if(config('wechat_report_levenl') == 1 &&
	...
	{		
		...
		//如果有配置代理这里就设置代理
		if(config('wechat_curl_proxy_host') != "0.0.0.0"
			&& config('wechat_curl_proxy_port') != 0){
			curl_setopt($ch,CURLOPT_PROXY, config('wechat_curl_proxy_host'));
			curl_setopt($ch,CURLOPT_PROXYPORT, config('wechat_curl_proxy_port'));
		}
