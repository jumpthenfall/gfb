...
use app\admin\model\CardAdsModel;
use app\admin\model\CardAdsPositionModel;
use think\Db;
...
class Ads extends Base
{
    ...
    public function index(){
        ...
        $count = Db('card_ads')->where($map)->count();//计算总页面
    ...
    public function add_ad()
    {
        if(request()->isAjax()){
			...
			$param['modified_time'] = date('Y-m-d H:i:s');
            $ad = new CardAdsModel();
            $flag = $ad->insertAd($param);
            return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
        }
        $position = new CardAdsPositionModel();
    ...
    {
        $id=input('param.id');
        $status = Db::name('card_ads')->where(array('id'=>$id))->value('status');//判断当前状态情况
        if($status==1)
        {
            $flag = Db::name('card_ads')->where(array('id'=>$id))->setField(['status'=>0]);
            return json(['code' => 1, 'data' => $flag['data'], 'msg' => '已禁止']);
        }
        else
        {
            $flag = Db::name('card_ads')->where(array('id'=>$id))->setField(['status'=>1]);
    ...
    public function index_position(){
        ...
        $ad = new CardAdsPositionModel();    
        $nowpage = input('get.page') ? input('get.page'):1;
        $limits = 10;// 获取总条数
        $count = Db::name('card_ads_position')->count();//计算总页面
        $allpage = intval(ceil($count / $limits));     
